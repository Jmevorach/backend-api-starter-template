"""ElastiCache (Valkey) Password Rotation Lambda.

This Lambda function rotates the Valkey/Redis password in Secrets Manager
and updates the ElastiCache user password.

Rotation Steps:
1. createSecret: Generate new password and store in AWSPENDING
2. setSecret: Update ElastiCache user password
3. testSecret: Verify new password works (optional)
4. finishSecret: Promote AWSPENDING to AWSCURRENT

Environment Variables:
- ELASTICACHE_USER_ID: The ElastiCache user ID to update
...
# ElastiCache passwords: 16-128 chars, printable ASCII except /, @, ", space
...
ENV_ELASTICACHE_USER_ID: Final[str] = "ELASTICACHE_USER_ID"
...
def _elasticache_client() -> ElastiCacheClient:
...
def _handle_set_secret(
    ...
    *,
    ...
    """Update the ElastiCache user with the new password."""
    ...
    user_id = os.environ.get(ENV_ELASTICACHE_USER_ID)
    if not user_id:
        msg = f"{ENV_ELASTICACHE_USER_ID} environment variable not set"
        raise ValueError(msg)

    # Update ElastiCache user password
    try:
        elasticache_client = _elasticache_client()
        elasticache_client.modify_user(
            UserId=user_id,
            Passwords=[new_password],
            NoPasswordRequired=False,
        )
        LOGGER.info("Updated ElastiCache user %s password", user_id)
    except ClientError:
        LOGGER.exception("Failed to update ElastiCache password")
        raise


def _handle_test_secret() -> None:
    """Test the new password (optional - we rely on API success)."""
    # For simplicity, we trust the ElastiCache modify operation succeeded
    # A more thorough test would attempt a Redis connection
    LOGGER.info("Test step: trusting ElastiCache modify operation succeeded")
...
def lambda_handler(event: RotationEvent, _context: object) -> None:
    """Handle Secrets Manager rotation steps for ElastiCache password."""
